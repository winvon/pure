...
<div class="content-wrap">
    <div class="login-box">
        <h1 style="text-align: center;padding-top:20px ;margin-bottom:20px;"><?= yii::t('frontend', '验证码登录') ?></h1>
        ...
        ])->passwordInput(['placeholder' => '请填写新密码']) ?>

        <div class="form-group">
            <?= Html::submitButton(yii::t('frontend', '登录'),
                [
...
//        var exten=$(".selected-flag")[0].title.split(":")[1].trim();
        if (checkPhone() == true) {//验证手机号码
            if (jishi == 1) {
                $.ajax({
                    type: "POST",
                    url: '<?=Url::to(['public/send-mobile-code-forget'])?>',
                    data: {
                        telephone: $("input[name='SignupForm[telephone]']").val(),
                        district: $(".selected-flag")[0].title
                    },
