    ...
    <div class="content">
        ...
        <header class="article-header">
            <h1 class="article-title"><a
                        href="<?= Url::to(['article/view', 'id' => $model->id]) ?>"><?= $model->title ?></a></h1>
            <div class="meta">
                ...
                <span class="muted"><i class="layui-icon layui-icon-username"></i> <a href="">admin</a></span>
                <time class="muted"><i
                            class="fa fa-clock-o"></i> <?= yii::$app->getFormatter()->asDate($model->created_at) ?>
                </time>
                <span class="muted"><i class="layui-icon layui-icon-read"></i>
                    <?= $model->scan_count * Yii::$app->params['base_read_time']; ?>
                </span>
                <span class="muted"><i class="layui-icon layui-icon-reply-fill"></i>
                    <a href="<?= Url::to([
                        'article/view',
                        'id' => $model->id
                    ]) ?>#comments">
        ...
        <article class="article-content">
            ...
            <p>
                ...
                <a href="<?= yii::$app->homeUrl ?>" data-original-title=""
            ...
            <div class="article-social">
                <a href="javascript:;" data-action="ding" data-id="<?= $model->id ?>"
        ...
        <footer class="article-footer">
            <div class="article-tags">
                ...
                <?php foreach ($model->articleTags as $tag) { ?>
                    <a href="<?= Url::to(['search/tag', 'tag' => $tag->value]) ?>" rel="tag" data-original-title=""
                       title=""><?= $tag->value ?></a>
                <?php } ?>
            </div>
        </footer>
            ...
            <form action="" method="post" id="commentform">
                ...
                <div class="comt-title" style="display: block;">
                    <div class="comt-avatar pull-left">
                        <img src="/static/images/guest.jpg" class="avatar avatar-108" height="50" width="50">
                    </div>
                    <div class="comt-author pull-left"><?php if (yii::$app->getUser()->getIsGuest()) {
                        ...
                        <a class="switch-author" href="javascript:void(0)" data-type="switch-author"
                           style="font-size:12px;"><?= yii::t('frontend', 'Change account') ?></a>
                    </div>
                ...
                <div class="comt">
                    <div class="comt-box">
                        ...
                        if (Yii::$app->request->isGet) {
                            echo $form->field($commentModel, 'content', ['template' => '{input}'])->textarea([
                        ...
                        <div class="comt-ctrl">
                            ...
                            <div class="comt-tips pull-right">
                                ...
                                <p style="display: none;"><input type="hidden" id="akismet_comment_nonce"
                                                                 name="akismet_comment_nonce" value="32920dc775"/></p>
                            ...
                            <span data-type="comment-insert-smilie" class="muted comt-smilie"><i
                                        class="fa fa-smile-o"></i> <?= yii::t('frontend', 'emoj') ?></span>
                            <span class="muted comt-mailme"><label for="comment_mail_notify" class="checkbox inline" style="padding-top:0">
                                <input type="checkbox" name="comment_mail_notify" id="comment_mail_notify"
                                       ...
                                       checked="checked"><?= yii::t('frontend', 'Send email at someone replied') ?></label>
                            </span>
                        </div>
                    </div>
                    <div class="comt-comterinfo" id="comment-author-info" style="display:none">
                        <h4><?= yii::t('frontend', 'Please fill') ?></h4>
                        <ul>
        ...
        <div id="postcomments">
            <div id="comments">
                ...
                <b><?= $model->comment_count ?></b><?= yii::t('frontend', 'person posted') ?>
            </div>
            <ol class="commentlist">
                ...
                foreach ($commentList as $v) {
                    ...
                    <li class="comment even thread-even depth-1 byuser comment-author-admin bypostauthor"
                        id="comment-<?= $v['id'] ?>">
                        <div class="c-avatar">
                            <img data-original="/static/images/guest.jpg"
                                 class="avatar avatar-108" height="50" width="50"
                                 src="/static/images/guest.jpg"
                                 style="display: block;">
                                    ...
                                    <li class="comment odd alt depth-2" id="comment-<?= $value['id'] ?>">
                                        <div class="c-avatar">
                                            <img data-original="/static/images/guest.jpg"
                                                 ...
                                                 style="display: block;">
                                            <div class="c-main"
                                                 id="div-comment-<?= $value['id'] ?>"><?= $value['content'] ?><br>
                                                <div class="c-meta">
                                                    <span class="c-author">
                                                        <a href="<?= $v['website_url'] ?>" rel="external nofollow"
                                                           class="url"
                                                           target="_blank"><?= empty($value['nickname']) ? yii::t('frontend', "Guest") : $value['nickname'] ?></a>
                                                    </span>
                                                    <?= yii::$app->getFormatter()->asDate($value['created_at']) ?>
