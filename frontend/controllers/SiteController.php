...
use frontend\models\form\UpdatePassword;
...
class SiteController extends FrontendController
{
    ...
    public function actionUpdatePassword()
    {
        ...
        if (Yii::$app->request->getMethod() == "POST") {
            ...
            if ($model->validate($data) && $model->load($data) && $model->updatePassword()) {
                Yii::$app->session->setFlash('success', '密码修改成功');
                return $this->goBack();
            }
        }
        return $this->render('updatePassword', compact('model'));
    }
    ...
    {
        $model = new SignupForm();
        $model->scenario = 'forget-password';
        if (Yii::$app->request->getMethod() == "POST") {
            $data = Yii::$app->request->post();
            if ($model->validate($data) && $model->load($data)) {
                if (!$model->validated()) {
                    return $this->render('forgetPassword', compact('model'));
                }
                $user = User::findOne(['telephone' => $data['SignupForm']['telephone']]);
                $user->scenario='forget-password';
                $user->setPassword($model->password);
                $user->save();
                Yii::$app->user->login($user);
                Yii::$app->session->setFlash('success', '密码修改成功'.$model->password);
                return $this->goHome();
            }
        }
        return $this->render('forgetPassword', compact('model'));
