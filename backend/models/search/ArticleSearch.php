...
class ArticleSearch extends Article
{
    ...
    {
        $query = CommonArticle::find()
            ->select([])
            ->where(['type' => $type]);

        if (Yii::$app->user->identity->type == User::TYPE_TEACHER) {
            $query->andWhere(['author_id' => Yii::$app->user->id]);
        }
        if ($type == self::ARTICLE) {
            $query->andWhere(['type' => self::ARTICLE]);
        }
        $query->with('category');
        ...
        $query->andFilterWhere(['like', 'title', $this->title])
            ...
            ->andFilterWhere(['like', 'author_name', $this->author_name]);//冗余字段便于搜索
        if ($this->thumb == 1) {
